...
uint32_t distance_sonar;
uint32_t unite_temps = 0;
uint32_t valeur_sonar;
uint8_t mur_detecte = 0;
UART_HandleTypeDef huart2;
uint8_t dataBluetooth;
#define SEUIL_3V 3723
uint16_t Vbatt;
uint8_t start_robot;
void avancer_robot_droit();
void detecter_mur();
void robot_tourner_droit();
void demarrer_roues();
...
int main(void)
{
  ...
  // démarrage le timer en mode Input Capture Interrupt
  HAL_TIM_IC_Start_IT(&htim1, TIM_CHANNEL_2);
  HAL_TIM_Base_Start_IT(&htim6);


  //on met les moteurs à l'arret
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  AnalogWDGConfig.HighThreshold = 4096-1;
...
void HAL_ADC_LevelOutOfWindowCallback(ADC_HandleTypeDef *hadc) {
    if (hadc->Instance == ADC1) {
        ...
        // Lire la valeur actuelle de l'ADC
        uint32_t adc_value = HAL_ADC_GetValue(hadc);
        float tension_batterie = ((float)adc_value / 4096) * 3.3;
        char msg[50];
        snprintf(msg, sizeof(msg), "Niveau batterie : %.2f V\n", tension_batterie);
...
void sonar_tourner_droite(){
		__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4, 2400-700);
}

void sonar_tourner_gauche(){
		__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4, 2400+700);
}

void sonar_mettre_milieu(){
...
void detecter_mur(){
	...
	if(valeur_sonar < 4000){
		unite_temps = 0;
		//arreter_roues();
		while(unite_temps < 4){
			sonar_tourner_droite();
		}
		uint32_t dd = valeur_sonar;
		unite_temps = 0;
		while(unite_temps < 4){
			sonar_tourner_gauche();
		}
		uint32_t dg = valeur_sonar;
		sonar_mettre_milieu();
		//demarrer_roues();
		if(dd>dg){
			while(valeur_sonar < 4000){
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
    if(GPIO_Pin == B1_Pin && start_robot == 0){
    	start_robot = 1;
    	demarrer_roues();
    }
    else if (GPIO_Pin == B1_Pin && start_robot == 1){
    	start_robot = 0;
    	arreter_roues();
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART3 && start_robot) {
